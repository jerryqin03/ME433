...
void pwmcontrol(float* in, float amplitude, float slope, float deadband);
...
#define scanline 30 // y coordinate of horizontal line that camera scans
...
int main()
{
    ...
    uint slice_num3 = pwm_gpio_to_slice_num(in3); // Get PWM slice number
    uint slice_num4 = pwm_gpio_to_slice_num(in4);
    float div = 60; // must be between 1-255
    pwm_set_clkdiv(slice_num1, div); // divider
    pwm_set_clkdiv(slice_num2, div);
    pwm_set_clkdiv(slice_num3, div); // divider
    ...
    float pwms[2];
    while(true){
        pwmcontrol(pwms, amp, slop, dead);
        pwm_set_gpio_level(in3, wrap*pwms[0]); // set the duty cycle to 50%
        pwm_set_gpio_level(in4, wrap*pwms[1]);
        ...
        //printf("%f", adcin);
        sprintf(buf, "Gain: %.2f, Slope: %.2f, Deadband: %.2f", amp, slop, dead);
        sprintf(message,"DCL: %.3f, DCR: %.3f", pwms[0], pwms[1]);
        drawMessage(0,0,buf);
...
void pwmcontrol(float* in, float amplitude, float slope, float deadband){
    ...
    // select one row of data and find the center of the path
    double com = 0;
    double mass = 0;
    for(int i=0; i<=IMAGESIZEX; i++){
        com = com + picture.g[scanline*IMAGESIZEX + i]*i;
        mass = mass + picture.g[scanline*IMAGESIZEX + i];
    }
    com = com/mass;

    // based on the difference from the row center to path center, output 2 motor pwms 
    double error = com - (IMAGESIZEX/2);
    if (fabs(error) < deadband){
        in[0] = amplitude;
        in[1] = amplitude;
    } else if(error > deadband){
        in[0] = amplitude;
        in[1] = fmax(amplitude - slope*error, 0);
    } else{
        in[0] = fmax(amplitude + slope*error, 0);
        in[1] = amplitude;
    }
    printf("%f, %f, %f, %f\n", com, error, in[0], in[1]);
}
...
void OV7670_write_register(uint8_t reg, uint8_t value){
    ...
    buf[0] = reg;
    buf[1] = value;
    i2c_write_blocking(I2C_PORT1, OV7670_ADDR, buf, 2, false);
...
uint8_t OV7670_read_register(uint8_t reg){
    uint8_t buf;
    i2c_write_blocking(I2C_PORT1, OV7670_ADDR, &reg, 1, false);  // true to keep master control of bus
    i2c_read_blocking(I2C_PORT1, OV7670_ADDR, &buf, 1, false);  // false - finished with bus
    return buf;
}
